...
Manages starting, health-checking, and stopping the kallip-daemon
...
DAEMON_BIN = BIN_DIR / "kallip-daemon"
RUN_BIN = BIN_DIR / "kallip-run"
AIFED_DIR = Path("/opt/aifed")
PID_FILE = Path("/tmp/kallip-daemon.pid")
...
PACKAGES: tuple[Package, ...] = (
    Package(
        ...
        target=INSTALL_DIR,
        bins=("kallip", "kallip-daemon", "kallip-run"),
        on_path=False,
    ),
    Package(
...
class DaemonManager:
    """Manages the kallip-daemon process lifecycle inside a Harbor container.

    The daemon must be running before ``kallip-run`` can be invoked,
