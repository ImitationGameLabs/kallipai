...
| `JUST_AGENT_ID`         | Per-agent shell (`routes/agent.rs`) | UUID of the current agent. Available inside agent shells. Read by the CLI for `skill` and `promote-request` subcommands, and passed as `created_by` when spawning subagents.                                                              |
