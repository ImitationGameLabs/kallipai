...
| `JUST_AGENT_AUTH_TOKEN` | Per-agent PTY (`routes/agent.rs`) | Generated 256-bit `sk-agent-…` authentication token. Injected into PTY sessions so the agent can call back to the daemon; the daemon stores and compares only its SHA-256. The CLI requires it; the TUI prompts interactively if unset. |
