...
```bash
just-agent remove <ID>
```

Removes the agent instance on the daemon.
...
the agent is already idle). Use `remove` to remove the agent entirely.
...
// Stream events, check status, remove
