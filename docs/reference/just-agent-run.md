...
```bash
just-agent-run --json --prompt "Summarize the project" | jq -r .assistant
```
...
```
$ just-agent-run --prompt "Summarize the project"
The project is a ...

agent a3f1b2c4-5678-90ab-cdef-1234567890ab finished (kept). Continue with: just-agent-run --agent a3f1b2c4-5678-90ab-cdef-1234567890ab --prompt "<prompt>"
$ just-agent-run --agent a3f1b2c4-5678-90ab-cdef-1234567890ab --prompt "and its license?"
```

Pass `--verbose` to watch the agent's reasoning and tool calls as it works:

```bash
just-agent-run --verbose --prompt "Summarize the project"
```
