...
# services `tagma` (the agent server) + `herald` (the relay connector), both
# run from packages.kallip-tagma-image.
#
...
in
{
  config = {
    ...
    # Tagma data, the agent workspace, and the herald's device key all live in
    ...
    services.tagma = {
      ...
      service.environment = {
        # The image bakes only PATH; the tagma-specific env lives here so it
    ...
    # reachable (a bad code crashloops -- documented). The tagma is co-located
    ...
    services.herald = {
      ...
      # nix-database layer); herald shares the tagma image with the tagma.
      build.image = lib.mkForce tagmaImage;
