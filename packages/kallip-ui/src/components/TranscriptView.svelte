...
>
  {#if lines.length === 0}
    ...
    <div
      class="h-full flex flex-col items-center justify-center text-center gap-4 py-10"
    >
      <h2 class="text-2xl font-semibold tracking-tight">KallipAI</h2>
  ...
  <!--
		Keyed by array index. This is only safe because the transcript reducer is
		append-only (it never reorders, removes, or mutates in place); if that ever
		changes, switch to a stable per-line id.
	-->
  {#each lines as line, i (i)}
    ...
    {:else if line.kind === "assistant"}
      <div class="border-l-2 border-primary-500 pl-3">
        ...
        {#if line.streaming}
          <!-- While streaming, render plain text + caret; switch to markdown once
               finalized to avoid re-parsing per delta and half-rendered fences. -->
    ...
    {:else if line.kind === "failoverExhausted"}
      <div class="space-y-0.5">
        ...
        {#if line.detail}<div class="text-xs opacity-60">
            {line.detail}
          </div>{/if}
      </div>
    {:else if line.kind === "streamDropped"}
      <div class="text-xs text-warning-500">
        stream dropped, retrying ({line.attempt}/{line.maxAttempts}) in {line.delaySecs}s:
        {line.error}
      </div>
    {/if}
  {/each}
