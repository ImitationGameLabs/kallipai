...
//!
//! Progressive disclosure — Harbor / `just-agent-run` set only env vars and ship no config
...
//! (the implicit profile derives it from `JUST_AGENT_CONTEXT_WINDOW_TOKENS`).
...
pub(crate) const PROFILES_FILE_ENV: &str = "JUST_AGENT_PROFILES_FILE";
...
/// backends. The daemon owns construction (see `just_agent_runtime::profile`).
#[derive(Debug)]
...
/// Load profile configuration: from `JUST_AGENT_PROFILES_FILE` (or a default path) if present,
...
/// derived from `JUST_AGENT_CONTEXT_WINDOW_TOKENS` (default `128_000`), so the env path and the
...
pub fn from_env() -> Result<ProfileConfig> {
    ...
    let max_context_window =
        crate::env_util::parse_env::<usize>("JUST_AGENT_CONTEXT_WINDOW_TOKENS")?
...
fn resolve_config_path() -> Result<Option<PathBuf>> {
    ...
    let path = dir.join("just-agent").join("profiles.toml");
...
mod tests {
    ...
    fn ds_env() -> [(&'static str, Option<&'static str>); 4] {
        [
            ...
            ("JUST_AGENT_CONTEXT_WINDOW_TOKENS", Some("200000")),
        ]
    }
