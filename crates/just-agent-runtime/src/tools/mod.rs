...
pub async fn build_tool_dispatch(inputs: ToolDispatchInputs<'_>) -> Result<ToolDispatcher> {
    ...
    let builder = {
        ...
        builder.access_source(move || {
            ...
            Ok(just_agent_shell::landlock::AccessDecision {
                read: just_agent_shell::landlock::ReadPolicy::Broad,
                writable,
                readonly_holes,
                hide_holes,
            })
...
/// [`just_agent_shell::landlock::AccessDecision::hide_holes`], which
/// `just-agent-shell::landlock::apply` realizes via libsandbox's
...
/// operator action (relocation or `JUST_AGENT_SECRET_HIDE_PATHS` of a parent
...
/// `~/.netrc`, …) — operators extend it via `JUST_AGENT_SECRET_HIDE_PATHS`, a
...
fn guest_hide_holes() -> Vec<PathBuf> {
    ...
    // so a custom JUST_AGENT_PROFILES_FILE location is covered, not just the default.
    if let Some(dir) = crate::profile::config::profiles_config_dir() {
        push_if_dir(dir, &mut out);
    }
    if let Some(extras) = std::env::var_os("JUST_AGENT_SECRET_HIDE_PATHS") {
...
mod tests {
    ...
    /// its contract — `is_dir()` filter, `JUST_AGENT_SECRET_HIDE_PATHS` `:`-split,
    ...
    fn guest_hide_holes_respects_is_dir_and_env_list() {
        ...
        // and an XDG_CONFIG_HOME with no `just-agent` subdir (profiles dir absent).
        ...
        temp_env::with_vars(
            [
                ...
                ("JUST_AGENT_SECRET_HIDE_PATHS", Some(list.as_os_str())),
            ],
            || {
