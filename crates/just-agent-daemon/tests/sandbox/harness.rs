...
//! setup, the scripted wiremock LLM, daemon + `just-agent-run` subprocess
...
pub fn unsupported() -> bool {
    ...
    // main.rs; this is the runtime kernel probe reused from just-agent-shell's
    // own test suite.
    if just_agent_shell::landlock::ensure_supported().is_err() {
...
/// Mirror of `just-agent-shell::landlock::tests::userns_unavailable`: read the
...
impl World {
    ...
    pub fn setup() -> Self {
        ...
        std::fs::create_dir_all(config_dir.join("just-agent")).unwrap();
...
async fn spawn_daemon(world: &World, permission_class: Option<&str>) -> DaemonProc {
    ...
    // caller before this; the daemon resolves it via JUST_AGENT_PROFILES_FILE.
    let mut env: Vec<(&str, String)> = vec![
        ("JUST_AGENT_OPERATOR_TOKEN", OPERATOR_TOKEN.into()),
        (
            "JUST_AGENT_DATA_DIR",
            world.data.path().display().to_string(),
        ),
        ("HOME", world.home_path().display().to_string()),
        ("XDG_CONFIG_HOME", world.config_dir.display().to_string()),
        (
            "JUST_AGENT_PROFILES_FILE",
            world.profiles_file().display().to_string(),
        ),
        ("JUST_AGENT_POLICY_PRESET", "allow-all".into()),
        ("JUST_AGENT_DAEMON_ADDR", format!("127.0.0.1:{port}")),
        ("JUST_AGENT_ADVERTISE_URL", url.clone()),
        (
            "PATH",
            format!(
                "{}:{}",
    ...
    let mut child = cmd.spawn().expect("spawn just-agent-daemon");
...
/// Parsed `just-agent-run --json` output (camelCase, per `main.rs::JsonObject`).
#[derive(Deserialize)]
...
pub async fn run_agent(daemon: &DaemonProc, workspace: &Path, max_rounds: usize) -> RunResult {
    let output = Command::new(resolve_bin("just-agent-run"))
        ...
        .env_clear()
        .envs([
            ("JUST_AGENT_DAEMON_URL", daemon.url.as_str()),
            ("JUST_AGENT_AUTH_TOKEN", OPERATOR_TOKEN),
            (
                "PATH",
                &format!(
                    "{}:{}",
        ...
        .output()
        .await
        .expect("spawn just-agent-run");

    let stdout = String::from_utf8_lossy(&output.stdout);
    let stderr = String::from_utf8_lossy(&output.stderr);
    if std::env::var("SANDBOX_VERBOSE").is_ok() {
        eprintln!(
            "=== just-agent-run stdout ===\n{stdout}\n=== just-agent-run stderr ===\n{stderr}"
        );
    }
    let line = stdout
        .lines()
    ...
    let result: RunResult = serde_json::from_str(line).unwrap_or_else(|_| {
        panic!(
            "just-agent-run produced no JSON; exit={}\n--- stdout ---\n{stdout}\n--- stderr ---\n{stderr}",
