...
//!
//! Surface for the `just-agent dirlock` CLI (which agents drive through
//! `bash_exec`). The daemon build unconditionally enables landlock enforcement
//! on Linux, so locks are mandatory there; on other platforms they are advisory.
//! See `just_agent_runtime::dirlock` for the coordinator and its invariants.
//!
//! # Authorization
//!
...
use just_agent_runtime::dirlock::AcquireOutcome;
