...
//! `KALLIP_CONTEXT_WINDOW_TOKENS` fallback ([`DEFAULT_CONTEXT_WINDOW_TOKENS`]), so neither
...
/// Default context window (tokens) for `KALLIP_CONTEXT_WINDOW_TOKENS` when unset. Shared by
